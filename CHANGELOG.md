...
- 将缩放拆分为两种：鼠标滚轮和预览按钮只改变 View 缩放，`Output size (%)` 则改变 Preview 中的图像内容并实际作用于最终导出。旋转仍会作用于 Preview 和导出；CLI 同步支持 `--zoom` 和 `--rotation`。
...
- 输出缩放采用中心裁切/黑边补齐，旋转自动扩展画布；比例尺在几何变换后重新绘制到右下角，并根据输出缩放和旋转后的物理像素尺寸计算长度。
- 批量文件切换时会在当前运行期间分别缓存通道、颜色、Min/Max/Gamma、Z 范围、比例尺和预览视图；切回文件时恢复其预览图、缩放、旋转和滚动位置，不再重置为默认值。
- 自动或手动刷新预览后，当前旋转角度会固化到预览图，比例尺再重新定位到右下角；新增 `Refresh Limit > Paused` 以暂停自动刷新，手动刷新仍可使用。
- `Channels` 中的颜色色块改为可点击 RGB 选择入口；选择后自动刷新预览，并同时作用于实际导出和批量通道匹配。
- 手动 RGB 被视为最终颜色，不再被“红转品红”二次修改；手动着色的单通道 TIFF 使用 RGB 输出，`export_info.json` 记录原颜色、最终颜色和是否手动覆盖。
- 预览状态会显示实际使用的金字塔层级；左键平移仍是不改变最终导出内容的屏幕查看操作。
