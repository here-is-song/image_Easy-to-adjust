...
界面支持打开 IMS、查看 metadata、选择通道和 1-based Z 范围、修改每个通道的 Display Min/Max/Gamma、切换 Merge/单通道预览、自动或手动比例尺，以及红色转品红色。每个通道的 Min、Max 和 Gamma 都同时提供精确数值框与横向滑块；任一方式调整后都会触发受刷新频率限制的预览更新。Gamma 范围与 Imaris 一致，为 `0.1–5.0`。预览和导出都在后台线程运行，避免大文件计算时冻结界面。
...
打开 IMS 后，`Objective` 折叠分组会显示自动检测结果、NA、浸液类型、Z spacing、来源和置信度。检测顺序为：原文件明确记录的物镜信息优先，其次使用标准化 Z voxel depth 与实验室 FV1200 calibration profile 比较。相对误差不超过 3% 为 High，3%–7% 为 Medium，超过 7% 不自动确认；20X/30X 候选过于接近时会降低置信度或要求手动确认。metadata 与 Z spacing 冲突时保留 metadata 结果并显示警告。

`Objective` 下拉菜单包含 `Auto`、`10X`、`20X`、`30X`、`60X` 和 `Unknown`。手动选择只覆盖本次处理和导出，不修改原始 IMS；自动检测结果与最终选择会分别写入 `export_info.json`。固定物镜参数集中保存在 `iea/fv1200_calibration.py`，以后可直接补充 XY pixel size/FOV calibration，不需要修改检测业务逻辑。
...
每次图像导出会在同一输出文件夹额外生成 `export_info.json`，记录输出格式、像素尺寸、DPI、源文件、Z 范围及物理位置、投影方式、比例尺及其粗细和文字大小、红转品红设置、每个通道实际使用的 Min/Max/Gamma、原始/输出颜色和输出文件路径。CLI 与 GUI 都会生成该记录。

每个 IMS 文件还会生成一个 `<源文件名>_PPT_summary.txt`，用于直接复制到 PowerPoint。程序会优先读取原始采集日期、显微镜厂家与型号和像素扫描速度；物镜使用 GUI 最终选择（Auto 或手动覆盖），Z 间隔使用本次检测所采用的标准化 voxel depth，并结合所选 Z 范围计算导出堆栈厚度。无法可靠识别的字段会显示为 `Not available`，不会根据文件名猜测。单文件 GUI 导出完成后，摘要内容也会直接显示在完成提示中。
